...
{
	BCXX_PWREN_LOW;						//¹Ø±ÕµçÔ´
	delay_ms(50);						//ÑÓÊ±´óÓÚ5ms
	BCXX_RST_HIGH;

	delay_ms(300);

	BCXX_PWREN_HIGH;					//´ò¿ªµçÔ´
	delay_ms(10);						//ÑÓÊ±´óÓÚ535us
	BCXX_RST_LOW;
...
//	delay_ms(5000);
	...
	if(!SendCmd("AT+CGATT?\r\n","+CGATT:1", 1000,5,TIMEOUT_5S))
	{
		...
		m53xx_get_AT_CSQ(&BcxxCsq);		//»ñÈ¡Í¨Ñ¶×´Ì¬ÖÊÁ¿
...
//ÉèÖÃIOTÆ½Ì¨IPºÍ¶Ë¿Ú HUAWEI IOT
...
//ÉèÖÃÍøÂçÊý¾Ý½ÓÊÕÄ£Ê½HUAWEI IOT
...
////ÏòUDP·þÎñÆ÷·¢ËÍÊý¾Ý²¢µÈ´ýÏìÓ¦Êý¾Ý
